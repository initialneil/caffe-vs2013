...
void MemoryDataLayer<Dtype>::AddMatVector(const vector<cv::Mat>& mat_vector,
    ...
    added_data_.Reshape(num, this->datum_channels_, this->datum_height_, this->datum_width_);
    added_label_.Reshape(num, 1, 1, 1);
    // Apply data transformations (mirror, scale, crop...)
    this->data_transformer_.Transform(mat_vector, &added_data_, this->mean_);
    // Copy Labels
    Dtype* top_label = added_label_.mutable_cpu_data();
...
void MemoryDataLayer<Dtype>::set_batch_size(int new_size) {
    ...
    added_data_.Reshape(batch_size_, this->datum_channels_, this->datum_height_, this->datum_width_);
    added_label_.Reshape(batch_size_, 1, 1, 1);
...
void MemoryDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  (*top)[0]->Reshape(batch_size_, this->datum_channels_, this->datum_height_, this->datum_width_);
  (*top)[1]->Reshape(batch_size_, 1, 1, 1);
